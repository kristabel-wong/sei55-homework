...
const planTrip = function (startLine, startStation, endLine, endStation ) {
    ...
    if ( startingLine.indexOf(startStation) > startingLine.indexOf(`Union Square`)){
    ...
    for(let i = newStartingLine.indexOf(startStation); i < newStartingLine.length; i++) {
        ...
        if(!sameLine && newStartingLine[i] === 'Union Square') {
            for (let j = newEndingLine.indexOf('Union Square'); j < newEndingLine.length; j++){
                stationsPassedBy.push(newEndingLine[j]);
                numberOfStationsPassed++;

                //if we reach endStation, then stop / return
                // joing command added to output stations in array
                if (newEndingLine[j] === endStation){
                    return {stations: stationsPassedBy.join(), stationsCount: numberOfStationsPassed }
                }
            }
        }
