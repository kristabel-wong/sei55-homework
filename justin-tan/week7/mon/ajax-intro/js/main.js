/*
    ...
    - Single Page Apps: instead of rendering HTML on  the server and sending it to the browser to be passively rendered, and re-loaded everytime we navigate to a new page or perform an action such as 'Liking' a post, 

    We want to load a base HTML page once, and use JS to load data 'in the background' from a remote server, and then manupulate the DOM to render this data into the browser

    For this to work, we need our servers to send us back not a full HTML page as our response, but just a piece of "raw" data - and the preferred format for this data is called "JSON", JavaScript Object Notation

    API: Application Programming Interface

    1st meaning: the set of methods provided by a library to access its features
    The ActiveRecord API: .find, .where, .create, .update, .destroy_all
    The jQuery API: $(), $().css(), $().html(), $().animate(), $().on()

    2nd meaning: a remote data source: numbersapi.com, themoviedb.org, generally returning "raw" data (as opposed to HTML pages) in JSON format
...
const lookupNumber = function(number){
    ...
    $('#results').html('Loading...'); // cleaer old results and show loading messages
    ...
    // properties with names 'onSOMETHING', and you save a function definition as the value of that 
    // property (ie. the value of that key); your function will then by run by the browser when that event happens
    ...
    // Tell is which URL to open, and HOW
...
$( function(){
    ...
    $('#submitButton').on('click', function(){
        ...
        // Use jQuery to get the valued typed into the form
